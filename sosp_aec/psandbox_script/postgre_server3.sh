...
sleep 5
sysbench --pgsql-db=postgres --pgsql-user=root --tables=1 --table-size=1000000 --threads=1 $SYSBEN_DIR/oltp_update_index.lua cleanup >> /dev/null
sysbench --pgsql-db=postgres --pgsql-user=root --tables=1 --table-size=1000000 --threads=1 $SYSBEN_DIR/oltp_update_index.lua prepare >> /dev/null
